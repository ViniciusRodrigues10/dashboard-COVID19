import dash # criar e gerenciar o dashbord 
from dash import dcc, html# responsavel por criar os componentes 
from dash.dependencies import Input, Output
import dash_bootstrap_components as dbc # formatar e colocar temas no dashbord

import plotly.express as px #criar graficos
import plotly.graph_objects as go #criar graficos mais aprimorados
...
# Pegando apenas dados dos estados e regiao do Brasil 
#criando o ("df_states.csv") e ("df_brasil.csv")
...
# ======================== Instalação do Dash ========================
# puxando tema " " / choropleth esta configurando o mapa  
...
# a cor do projeto / o tamanho que vai ser apresentado /e o estilo do mapa
fig.update_layout(
    paper_bgcolor="#F65C02",
    autosize=True,
...
#segundo grafico, especificando linha e coluna e a informação dos valores
...
# ======================== Construção Layout ========================
# Criando a linhas e colunas e botões dentro do layout e configurando o grafico
app.layout = dbc.Container(
    dbc.Row([
        dbc.Col([
            html.Div([
                html.Img(id="logo", src=app.get_asset_url("corona.png"), height=50),
                ...
                dbc.Card([
                    dbc.CardBody([
                        ...
                        html.H5(id="em-acompanhamento-text"),
                                        ...
                                        "color": "#FFFFFF"})# configuração do primeiro quadro
            ], md=4),
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        ...
                        html.H5(id="novos-casos-text"),
                                        ...
                                        "box-shadow": "0 4px 4px 0 rgba(0, 0, 0, 0.15), 0 4px 20px 0 rgba(0, 0, 0)",
                                        "color": "#FFFFFF"})# configuração do segundo quadro
            ], md=4),
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        ...
                        html.H5(id="obitos-na-data-text"),
                                        ...
                                        "box-shadow": "0 4px 4px 0 rgba(0, 0, 0, 0.15), 0 4px 20px 0 rgba(0, 0, 0)",
                                        "color": "#FFFFFF"})# configuração do terceiro quadro
            ], md=4),
        ]),
            html.Div([
        ...
        ], md=6 , style={"padding": "25px", "background-color": "#443DD8"}),# cor de fundo da tabela 

        dbc.Col([
            dcc.Loading(id="loading-1", type="default",
            children=[
                dcc.Graph(id="choropleth-map", figure=fig, style={"height": "170vh", "margin-right": "10px"})
                ]
            )
        ], md=6)
    ], className="g-0")
...
def update_map(date):
    ...
    fig = px.choropleth_mapbox(df_data_on_states, locations="estado", geojson=brazil_states, 
        ...
        zoom=3.5, color="casosAcumulado", color_continuous_scale="Redor", opacity=0.55, 
...
def update_location(click_data, n_clicks):
    ...
    if click_data is not None and changed_id != "loaction-button.n_clicks":
